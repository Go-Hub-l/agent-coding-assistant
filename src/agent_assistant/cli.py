...
def _write_artifact_files(session: Session, output_dir: Path) -> list[str]:
    """Write generated files from Coder and Tester artifacts to disk.
    ...
    for stage in ("coder", "tester"):
        artifact = session.artifacts.get(stage)
        if artifact is None:
            continue

        # Coder artifacts have "files", Tester artifacts have "test_files"
        key = "files" if stage == "coder" else "test_files"
        file_list = artifact.structured_data.get(key, [])
...
def build(
    ...
    output_dir: Optional[Path] = typer.Option(
        ...
        "--output-dir",
        "-o",
        help="Directory to write generated files to. Defaults to ./generated/",
    ),
) -> None:
    ...
    resolved_output = output_dir or Path.cwd() / "generated"
    ...
    if session.status == "completed":
        ...
        written = _write_artifact_files(session, resolved_output)
        if written:
            console.print()
            console.print(f"[bold green]Wrote {len(written)} files to {resolved_output}:[/bold green]")
            for f in written:
                console.print(f"  [green]✓[/green] {f}")
        else:
            console.print()
            console.print("[yellow]No files were generated by the pipeline.[/yellow]")
